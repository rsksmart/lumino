...
def test_is_balance_proof_usable_onchain_answer_is_false():
    channel_state = factories.make_channel_set(number_of_channels=1, token_address=factories.UNIT_TRANSFER_DESCRIPTION.initiator).channels.get(factories.UNIT_TRANSFER_DESCRIPTION.initiator)
    channel_state = channel_state.get(next(iter(channel_state)))
